...
class CustomerService(private val dal: AntaeusDal, private val telemetry: Telemetry) {
    ...
    fun pauseSubscription(id: Int): Boolean {
        if (dal.customerExists(id))
            telemetry.sendAlert("CUSTOMER",
                "Subscription paused for Customer id[$id]! Insufficient funds.")
        return dal.updateSubscriptionStatus(id, SubscriptionStatus.PAUSED)
    }
}
